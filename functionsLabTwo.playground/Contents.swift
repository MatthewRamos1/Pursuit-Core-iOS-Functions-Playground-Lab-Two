...
func fizzBuzz(upto: Int) -> [String] {
    ...
    for nums in 1...upto {
        switch true{
        case nums % 5 == 0 && nums % 3 == 0:
            replacedStr.append("FizzBuzz")
        case nums % 3 == 0:
            replacedStr.append("Fizz")
        case nums % 5 == 0:
            replacedStr.append("Buzz")
        default:
            replacedStr.append(String(nums))
...
for (inputOne, inputTwo, expectedOutput) in testCasesFour {
    let output = value(inputOne: inputOne, isGreaterThanAverageOf: inputTwo)
...
func secondSmallestValue (input: [Int]) -> Int {
...
for (input, expectedOutput) in testCasesFive {
    let output = secondSmallestValue(input:input)
    assert(output == expectedOutput, "Was expecting \(expectedOutput) for input \(input), but got \(output)")
}
